...
MergedSurvey <- MergedSurvey %>%
  mutate(Y1 =as.factor(if_else(factorLeniant >=1, 1,0 )), Y2=as.factor(if_else(factorLeniant >=2, 1,0 )), Y3 = as.factor(if_else(factorLeniant >=3, 1,0 )), Y4 = as.factor(if_else(factorLeniant >=4, 1,0 )), Y5 =as.factor(if_else(factorLeniant >=5, 1,0 )), Y6=as.factor(if_else(factorLeniant >=6, 1,0 )))
```
...
ggplot(data=factorResults, aes(y=Values, x = Group, shape =type)) + geom_point(size=5) + labs(title = "Estimated Change in Mental Health Scores", y="Treatment Effect",x="Scores Included in Estimation") + theme_bw() + theme(plot.title = element_text(hjust = 0.5)) + scale_shape_manual(values= c(16,4))
```
