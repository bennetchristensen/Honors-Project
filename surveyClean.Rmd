...
```{r}
#Convert the mental health screaner responses to binary, with some to all of the time being 1 and a little to none of the time being zero, and then summing them to create leniant health score
...
mergeCali2004 <- workCali2004 %>% dplyr::select("V1057","Race","AgeCat","Marriage", "Schooling", "Veteran", "MentalHealthScoreLeniant", "MentalHealthScoreStrict", "Year", "Treatment")
...
MergedSurvey <- MergedSurvey %>% mutate(Race = droplevels(Race), 
                                        ...
                                        factorLeniant = as.factor(MentalHealthScoreLeniant), 
...
```{r}
#Run linear models to look at relaationships between variables and leniant mental health score
lm(MentalHealthScoreLeniant~Year + Treatment +Year*Treatment, MergedSurvey) %>% summary() #Year and Treatment, some redundant variables in there
lm(MentalHealthScoreLeniant~Year + Treatment +Year*Treatment + Race + Schooling, MergedSurvey) %>% summary() # Year, treatment, race, and education
lm(MentalHealthScoreLeniant~Year + Treatment +Year*Treatment + Race + Schooling + AgeCat + Marriage + Veteran, MergedSurvey) %>% summary() #Year, treatment, race, education, age, marriage, and veteran
lm(MentalHealthScoreLeniant~Year + Treatment +Year*Treatment + Race + Schooling + AgeCat, MergedSurvey) %>% summary()#year, treatment, race, education, age
```


```{r}
#created plots that compare changes in mental health scores for the strict and leniant interpretations of the mental health scores
MergedSurvey %>% ggplot(aes(x=MentalHealthScoreStrict)) +
  geom_bar(na.rm=T) +
  facet_wrap(vars(Year,Treatment))
MergedSurvey %>% ggplot(aes(x=MentalHealthScoreLeniant)) +
  geom_bar(na.rm=T) +
  facet_wrap(vars(Year,Treatment))
```
...
```{r}
#Ordered logistic regresion using polr from the MASS package, not what we ended up going with
leniantOLog <- polr(factorLeniant ~ Year*Treatment + Year*Race + Year*Schooling + Year*AgeCat+ Year*Marriage + Year*Veteran, MergedSurvey, Hess =T) #the ordinal model for leniant interpretation
strictOLog <-polr(factorStrict ~ Year*Treatment + Year*Race + Year*Schooling + Year*AgeCat+ Year*Marriage + Year*Veteran, MergedSurvey, Hess =T) #the ordinal modelfor the strict interpretation
```
```{r}
...
(s <- with(MergedSurvey, summary(as.numeric(factorLeniant) ~ Year*Treatment + Year*Race + Year*Schooling + Year*AgeCat+ Year*Marriage + Year*Veteran, MergedSurvey, fun=sf)))
...
```
```{r}
#Created separate variables for each level of the ordinal logistic regression
MergedSurvey <- MergedSurvey %>%
  mutate(Y1 =as.factor(if_else(factorLeniant >=1, 1,0 )), 
         Y2=as.factor(if_else(factorLeniant >=2, 1,0 )), 
         Y3 = as.factor(if_else(factorLeniant >=3, 1,0 )), 
         Y4 = as.factor(if_else(factorLeniant >=4, 1,0 )), 
         Y5 =as.factor(if_else(factorLeniant >=5, 1,0 )), 
         Y6=as.factor(if_else(factorLeniant >=6, 1,0 )))
...
MergedSurvey <- MergedSurvey %>%
  mutate(Y1num =if_else(Y1=="1",1,0), 
         ...
         Y5num =if_else(MergedSurvey$Y1=="1",1,0), 
         Y6num=if_else(Y6=="1",1,0))
```

```{r}
#This is the full logistic modeling with logical statements to create the binary y variable
s1<-summary(glm((factorLeniant >=1) ~ Year*Treatment + Year*Race + Year*Schooling + Year*AgeCat+ Year*Marriage + Year*Veteran,family= "binomial", data = MergedSurvey))
s2<-summary(glm((factorLeniant >=2) ~ Year*Treatment + Year*Race + Year*Schooling + Year*AgeCat+ Year*Marriage + Year*Veteran,family= "binomial", data = MergedSurvey))
s3<-summary(glm((factorLeniant >=3) ~ Year*Treatment + Year*Race + Year*Schooling + Year*AgeCat+ Year*Marriage + Year*Veteran,family= "binomial", data = MergedSurvey))
s4<-summary(glm((factorLeniant >=4) ~ Year*Treatment + Year*Race + Year*Schooling + Year*AgeCat+ Year*Marriage + Year*Veteran,family= "binomial", data = MergedSurvey))
s5<-summary(glm((factorLeniant >=5) ~ Year*Treatment + Year*Race + Year*Schooling + Year*AgeCat+ Year*Marriage + Year*Veteran,family= "binomial", data = MergedSurvey))
s6<-summary(glm((factorLeniant >=6) ~ Year*Treatment + Year*Race + Year*Schooling + Year*AgeCat+ Year*Marriage + Year*Veteran,family= "binomial", data = MergedSurvey))
```
...
```{r}
#manually create data frame with estimates and standard deviations, where 23 is the coefficient and 65 is the standard deviation
factorResults <- data.frame(Values = c(m1$coefficients[23], m1$coefficients[23] + m1$coefficients[65], m1$coefficients[23] - m1$coefficients[65],
                                 m2$coefficients[23], m2$coefficients[23] + m2$coefficients[65], m2$coefficients[23] - m2$coefficients[65],
                                 m3$coefficients[23], m3$coefficients[23] + m3$coefficients[65], m3$coefficients[23] - m3$coefficients[65],
                                 m4$coefficients[23], m4$coefficients[23] + m4$coefficients[65], m4$coefficients[23] - m4$coefficients[65],
                                 m5$coefficients[23], m5$coefficients[23] + m5$coefficients[65], m5$coefficients[23] - m5$coefficients[65],
                                 m6$coefficients[23], m6$coefficients[23] + m6$coefficients[65], m6$coefficients[23] - m6$coefficients[65]),
...
```{r}
results <- data.frame(Values = c(s1$coefficients[23], s1$coefficients[23] + s1$coefficients[65], s1$coefficients[23] - s1$coefficients[65],
                                 s2$coefficients[23], s2$coefficients[23] + s2$coefficients[65], s2$coefficients[23] - s2$coefficients[65],
                                 s3$coefficients[23], s3$coefficients[23] + s3$coefficients[65], s3$coefficients[23] - s3$coefficients[65],
                                 s4$coefficients[23], s4$coefficients[23] + s4$coefficients[65], s4$coefficients[23] - s4$coefficients[65],
                                 s5$coefficients[23], s5$coefficients[23] + s5$coefficients[65], s5$coefficients[23] - s5$coefficients[65],
                                 s6$coefficients[23], s6$coefficients[23] + s6$coefficients[65], s6$coefficients[23] - s6$coefficients[65]),
                      type= c("Estimate","Std. Error","Std. Error","Estimate","Std. Error","Std. Error","Estimate","Std. Error","Std. Error","Estimate","Std. Error","Std. Error","Estimate","Std. Error","Std. Error","Estimate","Std. Error","Std. Error"), Group = c(">= 1",">= 1",">= 1",
                                                                                                         ">= 2",">= 2",">= 2",
                                                                                                         ">= 3",">= 3",">= 3",
                                                                                                         ">= 4",">= 4",">= 4",
...
```{r}
ggplot(data=results, aes(y=Values, x = Group, shape =type)) + geom_point(size=5) + labs(title = "Estimated Change in Mental Health Scores", y="Treatment Effect",x="Scores Included in Estimation") + theme_bw() + theme(plot.title = element_text(hjust = 0.5)) + scale_shape_manual(values= c(16,4))
ggsave("results.png")

ggplot(data=factorResults, aes(y=exp(Values), x = Group, shape =type)) + geom_point(size=5) + labs(title = "Estimated Change in Mental Health Scores", y="Treatment Effect",x="Scores Included in Estimation") + theme_bw() + theme(plot.title = element_text(hjust = 0.5)) + scale_shape_manual(values= c(16,4))
```
